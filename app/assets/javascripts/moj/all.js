...
if(MOJFrontend.dragAndDropSupported() && MOJFrontend.formDataSupported() && MOJFrontend.fileApiSupported()) {
  ...
  MOJFrontend.MultiFileUpload.prototype.uploadFiles = function(files) {
    ...
    for(var i = 0; i < files.length; i++) {
      fileNames.push({name: files[i].name, type: files[i].type})
      this.uploadFile(files[i]);
    }
    this.params.uploadFilesFinishedHook(this, fileNames)
  };
  ...
  MOJFrontend.MultiFileUpload.prototype.getDeleteButtonHtml = function(file) {
    var html = '<button class="moj-multi-file-upload__delete hods-button hods-button--secondary govuk-!-margin-bottom-0" type="button" name="delete" value="' + file.originalname + '">';
